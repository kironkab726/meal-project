...
# 제목 (길면 글씨를 줄여서 한 줄에 맞춤)
$size = 132
do { $tf = Font $size $true; $w = $g.MeasureString($Title, $tf).Width; $size -= 4 } while ($w -gt ($S - 130) -and $size -gt 60)
$g.DrawString($Title, $tf, (Brush '#4A3426'), 58, 160)
$g.DrawString($Sub, (Font 42 $false), (Brush '#7A6352'), 70, 330)
